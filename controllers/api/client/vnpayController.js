...
const createPaymentQr = async (req, res) => {
  try {
    ...
    await TempOrder.create({
      ...
      total_amount: total_amount || vnp_Amount / 100 // Chuyển đổi từ đơn vị VNPay (x100) về đơn vị thông thường
    });
  ...
  try {
    ...
    if (vnpResponse.vnp_ResponseCode === '00') {
      ...
      const newOrder = await Order.create({
        ...
        payment_status: 1,
        txn_ref: txnRef,
        discount_id: tempOrder.discount_id,
        discount_amount: tempOrder.discount_amount,
