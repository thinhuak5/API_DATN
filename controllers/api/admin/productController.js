const { Product, ProductImage, ProductVariation, OrderItem  } = require("../../../models"); // Đảm bảo import đúng các models
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    // Không cần xóa lại newProduct ở đây vì transaction đã rollback
    res.status(500).json({
    ...
    try {
        ...
        if (variations) {
            ...
            for (const incomingVariant of parsedVariations) {
                if (incomingVariant.id) { // Nếu có ID, đây là biến thể hiện có cần cập nhật
                    ...
                    await ProductVariation.update(
                        {
                            ...
                            price: incomingVariant.price !== "" ? parseFloat(incomingVariant.price) : null,
                ...
                } else { // Nếu không có ID, đây là biến thể mới cần tạo
            ...
            const variationIdsToDelete = existingVariations
                .filter((existingVariant) => !variationIdsToKeep.has(existingVariant.id))
            ...
            if (variationIdsToDelete.length > 0) {
                ...
                if (undeletableVariationIds.length > 0) {
                    ...
                    return res.status(400).json({
                        error: "Không thể xóa một số biến thể vì chúng được liên kết với các đơn hàng hiện có.",
                        details: details,
                    });
                }
...
exports.delete = async (req, res) => {
    ...
    try {
        ...
        const hasLinkedVariations = variationsInOrders.some(v => v.orderItems && v.orderItems.length > 0);
        ...
        if (hasLinkedVariations) {
            ...
            return res.status(400).json({
                error: "Không thể xóa sản phẩm này. Một hoặc nhiều biến thể của nó đang tồn tại trong các đơn hàng đã đặt.",
